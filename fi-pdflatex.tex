...
\thesissetup{
    ...
    TeXkeywords = {NLP, Neural networks},
    abstract    = {%
...
%results
I was able to create a pipeline and to pretrain several Czech ALBERT models. Even though their base performance was lower than the original English models, they achieved similar and in some cases surpassed the performance of the state of the art models for some Czech NLP tasks.  I am confident that with more powerful hardware and longer training time, these models could perform even better.

The resulting models can be easily adapted for a broad range o tasks using Keras.


% ####################################
% ####################################

\chapter{Artificial neural networks in NLP}
\section{Deep learning and NLP}
\cite{deepNLP}
shallow models svm
high dimensionality - One hot encoding
...
For many natural language processing (NLP) tasks, it is crucial to understand the underlying system of the language. This system is often complicated. It is also hard to gather a large amount of data for the specific task we want the model to learn. Because of this, models trained from scratch are usually not able to comprehend the language and are less accurate.

That is where the idea of transfer learning comes from. We can first pretrain a general language model on big and relatively easy-to-get corpora so that it can learn the inner language structure. Then we can fine-tune the model for the specific task. It has been shown that this approach can increase the quality of the resulting model.

In 2018, researchers from Google released a new language model called BERT~\parencite{bert}. It quickly rose to the top of the leaderboards of many English NLP tasks such as question answering or sentiment analysis. It is based on the idea of transfer learning and self-supervised learning, where the pretraining dataset can be automatically generated from raw text. Later, in 2019, they released a leaner version called ALBERT~\parencite{albert}, which is much smaller and achieves similar performance.


% ####################################
% ####################################

\chapter{Contextualized word embeddings}
polysemy

\section{ELMo}
bidirectional but lame
